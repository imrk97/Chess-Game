...
public class PAWN extends Piece {
	...
	boolean isValidPath(int dest_x, int dest_y) {
		...
		// moves two step
		if (x_distance == 2 && y_distance == 0 && dest_x < 8 && dest_x > -1
				&& Game_main_class.Board[dest_x][dest_y] == 0) {
			// if((y_pos == 1 && alliance == "white" &&
			// (Game_main_class.Board[x_pos][y_pos+1] == 0 ||
			// Game_main_class.Board[x_pos][y_pos+1] > 16)) || (y_pos == 6 && alliance ==
			// "black" && Game_main_class.Board[x_pos][y_pos-1] < 17))
			if (x_pos == 1 && turn == "white" && piece_id >= 9 && piece_id <= 16
					&& Game_main_class.Board[x_pos + 1][y_pos] == 0) {
				return true;
			} else if (x_pos == 6 && turn == "black" && piece_id >= 25 && piece_id <= 32
					&& Game_main_class.Board[x_pos - 1][y_pos] == 0) {
				return true;
			} else {
			...
			// goes diagonally for one step to cut opponent
				...
				if (turn == "white" && piece_id >= 9 && piece_id <= 16 && Game_main_class.Board[dest_x][dest_y] > 16) {
					return true;
						...
						&& Game_main_class.Board[dest_x][dest_y] < 17) {
					return true;
				} else {
